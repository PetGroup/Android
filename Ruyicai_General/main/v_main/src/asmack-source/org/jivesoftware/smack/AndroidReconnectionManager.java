...
public class AndroidReconnectionManager implements ConnectionListener {
    ...
    public AndroidReconnectionManager(Connection connection) {
        ...
        if (this.isReconnectionAllowed()) {
            ...
            reconnectionThread = new Thread() {
                ...
                public void run() {
                    ...
                    while (AndroidReconnectionManager.this.isReconnectionAllowed()) {
                                ...
                                remainingSeconds > 0)
                        {
                            try {
                                Thread.sleep(1000);
                                remainingSeconds--;
                                AndroidReconnectionManager.this
                                        .notifyAttemptToReconnectIn(remainingSeconds);
                            }
                            catch (InterruptedException e1) {
                                e1.printStackTrace();
                                // Notify the reconnection has failed
                                AndroidReconnectionManager.this.notifyReconnectionFailed(e1);
                            }
                        }

                        // Makes a reconnection attempt
                        try {
                            if (AndroidReconnectionManager.this.isReconnectionAllowed()) {
                                connection.connect();
                            }
                        }
                        catch (XMPPException e) {
                            // Fires the failed reconnection notification
                        	AndroidReconnectionManager.this.notifyReconnectionFailed(e);
                        }
                    }
                }
            };
